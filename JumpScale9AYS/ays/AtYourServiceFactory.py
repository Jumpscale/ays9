...
from inotify.constants import IN_MOVE, IN_ISDIR, IN_CREATE, IN_DELETE
import threading
if "." not in sys.path:
    sys.path.append(".")
...
class AtYourServiceFactory:
    ...
    def __init__(self):
        ...
        self.watcher = None
        self.watcher_mask = IN_MOVE | IN_ISDIR | IN_CREATE | IN_DELETE
            ...
            try:
                for event in self.watcher.event_gen():
                    ...
                    if event is not None:
                        ...
                        ays_repos_paths = self.aysRepos._repos.keys()
                        template_repos_paths = self.templateRepos._template_repos.keys()
                        if header.mask & IN_ISDIR:
                            path = path.decode()
                            try:
                                if path.startswith(tuple(template_repos_paths)):
                                    self.templateRepos.handle_fs_events(path, filename.decode(), event)
                                elif path.startswith(tuple(ays_repos_paths)):
                                    self.aysRepos.handle_fs_events(path, filename.decode(), event)
                            except Exception as e:
                                print(e)
                else:
                    break
            except InotifyError as e:
                self.logger.error("inotify error %s" % e)
